...
case class SpArrowSpecialLimits(spark: SparkSession) extends SpArrowStrategy {
  override def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
    case ReturnAnswer(rootPlan) => rootPlan match {
      case Limit(IntegerLiteral(limit), child) =>
        ArrowCollectLimitExec(limit, ArrowPlanLater(child.asInstanceOf[LogicalPlan with ArrowLimit])) :: Nil
