...
import org.apache.spark.rdd.sparrow.ArrowInternalColumn
...
class SimpleArrowFileFormat extends ArrowFileFormat with DataSourceRegister with Serializable with Logging {
  ...
  override def buildArrowReaderWithPartitionValues(sparkSession: SparkSession, dataSchema: StructType, partitionSchema: StructType, requiredSchema: StructType, filters: Seq[Filter], options: Map[String, String], hadoopConf: Configuration, rddId: Long): PartitionedFile => Iterator[ArrowInternalColumn] = {
    (file: PartitionedFile) => { new ParquetReaderIterator(file, rootAllocator, rddId)}
  }
}
