...
//    fw.write(s"# Experiment repeated $nr_runs times, with running times in seconds\n")
    ...
    handler.process(new Directory(Paths.get("", "data", "numbers").toFile))
    print(s"row count: ${handler.getVectorSchemaRoot.getRowCount}\n")

//    0 until nr_runs foreach { _ =>
//      EvaluationSuite.minimumValue(sc, fw, s"${Paths.get("", "data", "numbers")}", 10*1000*1000)
//    }
